...
aws_config_file = "PiBot-provisioning-app/src/app/awsconfig.ts"
...
for pool in user_pools['UserPools']:
	if pool['Name'] == 'provisioning-userpool':
...
for app in user_pool_clients['UserPoolClients']:
	if app['ClientName'] == 'provisioning_client':
		app_id = app['ClientId']
...
for idp in id_pools['IdentityPools']:
	if idp['IdentityPoolName'] == "provisioning_id_pool":
